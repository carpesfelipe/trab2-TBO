...
void merge(Linha *vetor_proximas_linhas, int P, FILE **arquivo_fonte, FILE *arquivo_destino, char **campos_juncao, int qtd_campos_juncao, int tamanho_atual_bloco, int *linhas_lidas_bloco, int n_linhas_arquivo_entrada){
    ...
    while (1)
    {
        ...
        vetor_proximas_linhas[indice_menor] = le_proxima_linha(arquivo_fonte[indice_menor], &vetor_proximas_linhas[indice_menor], campos_juncao, qtd_campos_juncao, &linhas_lidas_bloco[indice_menor], tamanho_atual_bloco);
    }
}
...
void abre_arquivos_temporarios(FILE **arquivos_fontes_para_esta_rodada, int P, int inicio_arquivo_fonte, int indice_arquivo_destino_atual, char *arq, FILE **arquivo_destino_para_esta_rodada, char *nome){
    ...
    *arquivo_destino_para_esta_rodada = fopen(nome, "a");
...
void ordena_blocos(FILE **temp_files, int P, int M, char *arquivo_in, char **L, int tam, char *arq)
{
    ...
    while (tamanho_bloco_ordenado_atual < n_linhas_arquivo_entrada) // Enquanto todos os dados não estiverem em um único bloco ordenado
    {
        ...
        for (int i = 0; i < ceil(num_blocos_ord_totais / (double)P); i++)
        {
            ...
            inicializa_vetor_proximas_linhas(arquivos_fontes_para_esta_rodada, P, vetor_proximas_linhas, L, tam, linhas_lidas_do_bloco, tamanho_bloco_ordenado_atual, i);

            merge(vetor_proximas_linhas, P, arquivos_fontes_para_esta_rodada, arquivo_destino_para_esta_rodada, L, tam, tamanho_bloco_ordenado_atual, linhas_lidas_do_bloco, n_linhas_arquivo_entrada);

            libera_vetor_proximas_linhas(vetor_proximas_linhas, P);

            fecha_arquivos_temporarios(arquivos_fontes_para_esta_rodada, P, arquivo_destino_para_esta_rodada);
        ...
        indice_arquivo_destino_atual = troca_arquivos(&inicio_arquivo_fonte, &inicio_arquivo_destino, P); // alterna arquivos fonte e destino
        ...
        limpa_arquivos_destino(temp_files, inicio_arquivo_destino, P, arq); // limpa os arquivos que vamos escrever
    }
